...
import argparse
from CustomDataset import ESC50Dataset
from L3Net import L3NetAudioOnly
...
def get_arguments():
    ...
    parser.add_argument('--data_dir', default="D:\\datasets\\ESC-50-master")
...
def main(args):
    ...
    audio_transform = nn.Sequential(
        torchaudio.transforms.Resample(44100, 48000),
        torchaudio.transforms.Spectrogram(n_fft=512)
    )

    full_dataset = ESC50Dataset(data_dir, audio_transform)
    ...
    model = L3NetAudioOnly(L3Net_weights).to(device)
    # model.load_state_dict(torch.load(manifest_dir / 'audioonly_best_model_by_accuracy.pth'))
    ...
    for t in range(epochs):
        ...
        if test_accuracy > best_accuracy:
            ...
            torch.save(model.state_dict(), manifest_dir / 'audioonly_best_model_by_accuracy.pth')
        if test_loss < best_loss:
            best_loss = test_loss
            print("Saving Best Model by loss...")
            torch.save(model.state_dict(), manifest_dir / 'audioonly_best_model_by_loss.pth')
    torch.save(model.state_dict(), manifest_dir / 'audioonly_last_model.pth')
    print("Done!")
    ...
    df.to_csv(manifest_dir / 'esc50history.csv', index=False)
